...
#include "ae.h"
#include "/home/fangjian/c++project/redis-2.8.7/deps/hiredis/hiredis.h"
#include "sds.h"
#include "adlist.h"
#include "zmalloc.h"
...
typedef struct _client {
    ...
    unsigned int randlen;         /* Number of pointers in client->randptr */
    unsigned int randfree;        /* Number of unused pointers in client->randptr */
...
static void randomizeClientKey(client c) {
    ...
    for (i = 0; i < c->randlen; i++) {
        ...
        unsigned int r = random() % config.randomkeys_keyspacelen;
        unsigned int j;
 ...
 * if needed. */
static client createClient(char *cmd, unsigned int len, client from) {
    int j;
...
static void createMissingClients(client c) {
    int n = 0;
    char *buf = c->obuf;
    unsigned int buflen = sdslen(c->obuf);
...
int parseOptions(int argc, const char **argv)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if (!strcmp(argv[i],"-c")) //模拟出来的客户端的个数
        {
            if (lastarg) goto invalid;
            config.numclients = atoi(argv[++i]);
        }
        else if (!strcmp(argv[i],"-n"))//总的请求数量
        {
            if (lastarg) goto invalid;
        ...
        else if (!strcmp(argv[i],"-h")) //主机IP
        {
            if (lastarg) goto invalid;
            config.hostip = strdup(argv[++i]);
        }
        else if (!strcmp(argv[i],"-p")) //主机端口
        {
            if (lastarg) goto invalid;
