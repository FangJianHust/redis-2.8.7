		...
		robj *ok_reply, robj *abort_reply)
{
    ...
    setKey(c->db,key,val);//把数据添加到数据库中
    server.dirty++;
    //设置该数据的过期时间
...
void setCommand(redisClient *c) {
    ...
    c->argv[2] = tryObjectEncoding(c->argv[2]);//对客户端发送来的数据进行编码
...
{
    robj *o;
    /* 以读数据为目的查询key函数,并且如果该key不存在,则在该函数中做不存在的回包处理，如果该key存在, 则返回该key对应的数据 */
    if ((o = lookupKeyReadOrReply(c,c->argv[1],shared.nullbulk)) == NULL)
        return REDIS_OK;
    if (o->type != REDIS_STRING)//get命令返回的value类型必须是字符串类型
    {
