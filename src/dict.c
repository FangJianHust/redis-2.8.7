...
#include <sys/time.h>
#include <limits.h>
static int dict_can_resize = 1;//记录hash表是否可以自动扩容
...
/* Reset a hash table already initialized with ht_init(). This function should only be called by ht_destroy(). */
...
int dictResize(dict *d)
{
    ...
    return dictExpand(d, minimal);//进行真正的扩容
}
...
int dictExpand(dict *d, unsigned long size)
{
    ...
    unsigned long realsize = _dictNextPower(size);//选择一个满足需求的2的指数
    ...
    n.table = zcalloc(realsize*sizeof(dictEntry*));//分配空间，创建新的hash表
    n.used = 0;
    /* 如果ht[0]为空，则直接将hash表赋值给ht[0],此时表示创建一个新的hash表 */
    if (d->ht[0].table == NULL) {
        d->ht[0] = n;
        return DICT_OK;
    }
    /* 将hash表赋值给ht[1],并启动拷贝过程，将ht[0]的记录逐个桶的拷贝到ht[1]中，置rehashidx=0表明正在扩容，
     * 且待拷贝的桶为ht[0]->table[rehashidx]*/
    d->ht[1] = n;
    d->rehashidx = 0;
    return DICT_OK;
}

/* 进行真正的拷贝扩容，将ht[0]中所有的记录都拷贝到ht[1]中 */
int dictRehash(dict *d, int n)
{
    ...
    while(n--)//本次扩容n个桶
    {
        ...
        de = d->ht[0].table[d->rehashidx];//待拷贝的桶
...
/* 为了节省时间，只reshash一个桶，一般是正在操作的桶
 * This function performs just a step of rehashing, and only if there are no safe iterators bound to our hash table. When
 * we have iterators in the middle of a rehashing we can't mess with the two hash tables otherwise some element can be
 * missed or duplicated.
 *
 * This function is called by common lookup or update operations in the dictionary so that the hash table automatically
 * migrates from H1 to H2 while it is actively used. */
...
/* Low level add. This function adds the entry but instead of setting a value returns the dictEntry structure to the user,
 * that will make sure to fill the value field as he wishes.
 *
 * This function is also directly exposed to user API to be called mainly in order to store non-pointers inside the hash
 * value, example:
 *
 * entry = dictAddRaw(dict,mykey);
 * if (entry != NULL) dictSetSignedIntegerVal(entry,1000);
 *
 * 在hash表上添加一个节点，给该节点赋上key，但是不赋值
 */
...
/* 原来的key不存在，则添加，返回1；如果存在，则更新，返回0 */
int dictReplace(dict *d, void *key, void *val)
{
    ...
    /* Set the new value and free the old one. Note that it is important to do that in this order, as the value may
     * just be exactly the same as the previous one. In this context, think to reference counting, you want to
     * increment (set), and then decrement (free), and not the reverse. */
    auxentry = *entry;
    dictSetVal(d, entry, val);
    dictFreeVal(d, &auxentry);//释放原来的值
    return 0;
}
...
/* 查找key所在的hash表的节点，如果没有正在rehashsing，在在table[0]查找，否则，两个都查找 */
...
dictEntry *dictNext(dictIterator *iter)
{
    while (1)
    {
        if (iter->entry == NULL)
        {
            dictht *ht = &iter->d->ht[iter->table];//获得当前hash表
 ...
 *
 * Note that if we are in the process of rehashing the hash table, the index is always returned in the context of the
 * second (new) hash table. */
static int _dictKeyIndex(dict *d, const void *key)
{
    ...
    for (table = 0; table <= 1; table++)
    {
        ...
        he = d->ht[table].table[idx];//指向该桶对应的第一条记录
        while(he)
        {
            if (dictCompareKeys(d, key, he->key)) return -1;//找到该key对应的记录，返回-1
            he = he->next;
        }
        if (!dictIsRehashing(d)) break;//如果没有在进行rehash，则此处返回，即找到的hash0的桶，否则，查找hash1的桶
    }
    return idx;
}
