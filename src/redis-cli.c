...
#include "sds.h"
#include "zmalloc.h"
#include "/home/fangjian/c++project/redis-2.8.7/deps/linenoise/linenoise.h"
#include "help.h"
#include "anet.h"
#include "ae.h"
...
static int cliSendCommand(int argc, char **argv, int repeat) {
    ...
    argvlen = malloc(argc*sizeof(unsigned int));
    for (j = 0; j < argc; j++)
    ...
    while(!done) {
        ...
        if (mask & AE_WRITABLE) {
            while(1) {
                ...
                if (obuf_len != 0) {
                    unsigned int nwritten = write(fd,obuf+obuf_pos,obuf_len);
                ...
                if (obuf_len == 0 && !eof) {
                    unsigned int nread = read(STDIN_FILENO,obuf,sizeof(obuf));
