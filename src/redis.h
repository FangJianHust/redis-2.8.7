...
#include "/home/fangjian/c++project/redis-2.8.7/deps/lua/src/lua.h"
/* Error codes */
#define REDIS_OK                0
#define REDIS_ERR               -1
...
#define REDIS_HASH 4			/* hash表 */

/* Objects encoding. Some kind of objects like Strings and Hashes can be internally represented in multiple ways.
...
#define REDIS_MULTI (1<<3)   /* 当前客户端处于multi状态 */
...
{
    unsigned type:4;		 //表示value的类型：stringm,list,set
    unsigned notused:2;     /* Not used */
    unsigned encoding:4;	 /* type所代表的类型的底层实现，redis称为编码方式 */
...
/* Redis 中的每个数据库,都由一个该结构表示，配置文件中默认有16个 */
typedef struct redisDb
{
    ...
    dict *watched_keys;         /* 用于实现WATCH命令:字典的键是这个数据库监视的键，字典的值则是一个链表，链表中保存了所有监视这个键的客户端 */
...
{
    robj **argv;				// 执行命令的所有 key 对象
    int argc;					// 参数的数量
    struct redisCommand *cmd;  // 被执行的命令
} multiCmd;

typedef struct multiState {
    multiCmd *commands;     /* 事务状态下保存的所有命令*/
...
/* 用来保存每个客户端的状态、数据等信息，所有的客户端用链表连接起来 */
typedef struct redisClient
{
    int fd;					/* connect fd */
    redisDb *db;	/* 客户端当前操作的db，服务器默认创建16个db，每个客户端可以使用select命令来选择当前操作的db，保存在db中，默认是0号db */
    int dictid;
    robj *name;             /* As set by CLIENT SETNAME */
    sds querybuf;			/* 客户端发送来的未解析的数据 */
    unsigned int querybuf_peak;   /* Recent (100ms or more) peak of querybuf size */
    int argc;
    robj **argv;	/* 存放客户端发送来的命令，对于set，argv[0]="set",argv[1]=key,argv[2]=value */
    struct redisCommand *cmd, *lastcmd;
    int reqtype;			/* 客户端的请求类型：REDIS_REQ_MULTIBULK代表multibulk 协议；REDIS_REQ_INLINE代表inline 协议 */
    int multibulklen;       /* number of multi bulk arguments left to read */
    long bulklen;           /* length of bulk argument in multi bulk request */
    list *reply;			/* 发送给客户端的存放在链表中的数据，即响应数据先放在buf中，剩下的存放在该链表中 */
    unsigned long reply_bytes; /* 响应数据链表中待发送的数据字节数 */
    int sentlen;            /* Amount of bytes already sent in the current buffer or object being sent. */
    ...
    list *watched_keys;     /* Keys WATCHED for MULTI/EXEC CAS，每一个元素是一个watchedKey */
...
/* 共享对象，它用于全局文本信息的保存，避免每次发送固定格式的信息给clients都需要创建一个新的字符串 */
...
/* 保存服务器的相关信息：配置信息，统计信息，服务器状态信息等 */
struct redisServer
{
    /* General */
    char *configfile;           /* Absolute config file path, or NULL */
    int hz;                     /* serverCron() calls frequency in hertz */
    redisDb *db;				 /* 默认创建16个db，每个客户端可以使用select命令来选择当前操作的db，保存在c->db中 */
    dict *commands;             /* Command table */
    dict *orig_commands;        /* Command table before command renaming. */
    aeEventLoop *el;
    unsigned lruclock:22;       /* Clock incrementing every minute, for LRU */
    unsigned lruclock_padding:10;
    int shutdown_asap;          /* SHUTDOWN needed ASAP(as soon as possible) */
    ...
    int sentinel_mode;          /* redis是否以Sentinel模式启动 */
    /* Networking */
    int port;                   /* TCP listening port，默认为6379 */
    int tcp_backlog;            /* TCP listen() backlog */
    char *bindaddr[REDIS_BINDADDR_MAX]; /* Addresses we should bind to */
    int bindaddr_count;         /* Number of addresses in server.bindaddr[] */
    char *unixsocket;           /* UNIX socket path */
    mode_t unixsocketperm;      /* UNIX socket permission */
    int ipfd[REDIS_BINDADDR_MAX]; /* TCP socket 监听描述符 */
    ...
    unsigned int stat_peak_memory;        /* Max used memory record */
    ...
    unsigned int client_max_querybuf_len; /* Limit for client query buffer length */
    int dbnum;                      /* 该server中的redisDb的总数，默认是16 */
    ...
    redisClient *master;     		/* Client that is master for this slave，当前是以slave方式启动的 */
    ...
    /* Limits */
    unsigned int maxclients;        /* 同时访问的最大客户端数 */
    ...
    unsigned int hash_max_ziplist_entries;
    unsigned int hash_max_ziplist_value;
    unsigned int list_max_ziplist_entries;
    unsigned int list_max_ziplist_value;
    unsigned int set_max_intset_entries;
    unsigned int zset_max_ziplist_entries;
    unsigned int zset_max_ziplist_value;
    time_t unixtime;        /* Unix time sampled every cron cycle. */
    long long mstime;       /* Like 'unixtime' but with milliseconds resolution. */
    /* Pubsub */
    dict *pubsub_channels;  /* 每一个键代表一个可以被订阅的channel，每一个channel关联的value链表代表订阅该channel的client */
...
unsigned int redisPopcount(void *s, long count);
...
robj *createStringObject(char *ptr, unsigned int len);
robj *dupStringObject(robj *o);
int isObjectRepresentableAsLongLong(robj *o, long long *llongval);
robj *tryObjectEncoding(robj *o);
robj *getDecodedObject(robj *o);
unsigned int stringObjectLen(robj *o);
...
unsigned int syncWrite(int fd, char *ptr, unsigned int size, long long timeout);
unsigned int syncRead(int fd, char *ptr, unsigned int size, long long timeout);
unsigned int syncReadLine(int fd, char *ptr, unsigned int size, long long timeout);
...
#if defined(__GNUC__)
void *calloc(unsigned int count, unsigned int size) __attribute__ ((deprecated));
void free(void *ptr) __attribute__ ((deprecated));
void *malloc(unsigned int size) __attribute__ ((deprecated));
void *realloc(void *ptr, unsigned int size) __attribute__ ((deprecated));
#endif
...
void redisLogHexDump(int level, char *descr, void *value, unsigned int len);
